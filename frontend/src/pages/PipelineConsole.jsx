...
import { useEffect, useState } from 'react'
import { RefreshCw, CheckCircle, XCircle, Clock, ChevronRight, Activity, Database } from 'lucide-react'
import { getAirflowDagRuns, getAirflowTaskInstances } from '../services/api'

// 1. DYNAMIC CONFIGURATION: Define both pipelines here
const PIPELINE_CONFIGS = {
  ddd_retrain_pipeline: {
    id: 'ddd_retrain_pipeline',
    tabLabel: 'Retraining',
    icon: Activity,
    title: 'Retraining Pipeline',
    subtitle: 'Airflow DAG — ddd_retrain_pipeline · MLflow · FastAPI',
    stages: [
      { id: 't0_check_retrain',       label: 'Check Conditions',   tool: 'Python',  color: '#06B6D4' },
      { id: 't1_pull_data',           label: 'Pull Data',          tool: 'DVC',     color: '#A78BFA' },
      { id: 't2_feature_engineering', label: 'Feature Eng',        tool: 'PySpark', color: '#10B981' },
      { id: 't3_train_xgboost',       label: 'Train XGBoost',      tool: 'XGBoost', color: '#F59E0B' },
      { id: 't4_train_svm',           label: 'Train SVM',          tool: 'sklearn', color: '#F59E0B' },
      { id: 't5_evaluate_models',     label: 'Evaluate Models',    tool: 'Python',  color: '#06B6D4' },
      { id: 't6_promote_if_better',   label: 'MLflow Promote',     tool: 'MLflow',  color: '#3B82F6' },
      { id: 't7_reload_model_server', label: 'Hot Reload',         tool: 'FastAPI', color: '#10B981' },
      { id: 't8_pipeline_report',     label: 'Pipeline Report',    tool: 'Python',  color: '#A78BFA' },
    ]
  },
  ddd_data_pipeline: {
    id: 'ddd_data_pipeline',
    tabLabel: 'Data Processing',
    icon: Database,
    title: 'Data Pipeline Console',
    subtitle: 'Airflow DAG — ddd_data_pipeline · DVC · PySpark',
    stages: [
      { id: 't0_validate_raw_data',   label: 'Validate Raw Data',  tool: 'Python',    color: '#06B6D4' },
      { id: 't1_extract_frames',      label: 'Extract Frames',     tool: 'OpenCV',    color: '#06B6D4' },
      { id: 't2_extract_landmarks',   label: 'Extract Landmarks',  tool: 'MediaPipe', color: '#06B6D4' },
...
export default function PipelineConsole() {
  // 2. NEW STATE: Track which pipeline is currently active
  const [activePipeline, setActivePipeline] = useState('ddd_retrain_pipeline')
  ...
  const load = async (pipelineId = activePipeline) => {
    setRefreshing(true)
    try {
      const data = await getAirflowDagRuns(pipelineId)
      const runs = data.dag_runs || []
      setDagRuns(runs)
      
      // Auto-select the most recent run for this pipeline
      const latest = runs[0]
      if (latest) {
        setSelectedRun(latest)
        const taskData = await getAirflowTaskInstances(pipelineId, latest.dag_run_id)
        setTasks(taskData.task_instances || [])
      } else {
        setSelectedRun(null)
        setTasks([])
  ...
  // Reload data whenever the active pipeline tab changes
  useEffect(() => {
    setLoading(true)
    setSelectedRun(null) // Clear old UI immediately
    setTasks([])
    load(activePipeline)
  }, [activePipeline])
  ...
  const selectRun = async (run) => {
    setSelectedRun(run)
    try {
      const data = await getAirflowTaskInstances(activePipeline, run.dag_run_id)
      setTasks(data.task_instances || [])
    } catch { setTasks([]) }
  }
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <div className="label" style={{ marginTop: 4 }}>{config.subtitle}</div>
          
          {/* Tab Switcher */}
          <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
            {Object.values(PIPELINE_CONFIGS).map(p => {
              ...
              return (
                <div key={stage.id} style={{ display: 'flex', alignItems: 'center' }}>
                  <div style={{
                    ...
                    gap: 8, padding: '12px 14px',
                    background: state === 'running'
                      ? `rgba(245,158,11,0.08)` : `rgba(${color.includes('#10') ? '16,185,129' : color.includes('#06') ? '6,182,212' : color.includes('#A7') ? '139,92,246' : color.includes('#3B') ? '59,130,246' : color.includes('#F5') ? '245,158,11' : '255,59,59'},0.06)`,
                    border: `1px solid ${color}44`,
                    borderRadius: 'var(--radius)',
                    minWidth: 110,
                    transition: 'all 0.3s ease',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 20 }}>
        ...
        <div className="card" style={{ height: 'fit-content' }}>
          <div className="label" style={{ marginBottom: 12 }}>Recent Runs</div>
          {loading ? (
            <div style={{ color: 'var(--text-muted)', fontSize: 12 }}>Loading {config.tabLabel} history...</div>
          ) : dagRuns.length === 0 ? (
            <div style={{ color: 'var(--text-muted)', fontSize: 12 }}>
              No runs found.<br />Trigger a run from Airflow UI at<br />
              <span style={{ color: 'var(--cyan)' }}>localhost:8080</span>
            ...
            dagRuns.map(run => {
              ...
              return (
                <div key={run.dag_run_id} onClick={() => selectRun(run)}
                  ...
                  <div style={{ fontSize: 10, color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)' }}>
                    {new Date(run.execution_date).toLocaleDateString()} {new Date(run.execution_date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                  </div>
                  <div style={{ fontSize: 9, color: 'var(--text-muted)', marginTop: 2 }}>
                    {run.dag_run_id.split('__').pop()?.slice(0, 16)}
                  </div>
        ...
        <div className="card">
              ...
              {config.stages.map(stage => {
                ...
                return (
                  <div key={stage.id} style={{
                    ...
                    padding: '10px 14px', borderRadius: 'var(--radius)',
                    background: 'var(--bg-elevated)',
                    border: `1px solid ${state === 'success' ? '#10B98122' : state === 'failed' ? '#FF3B3B22' : state === 'skipped' ? '#3A506844' : 'var(--border)'}`,
                  }}>
                    <StatusDot state={state} size={13} />
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 12, color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>
          ...
          <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid var(--border)', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {[
              { label: 'Airflow UI', url: 'http://localhost:8080', color: 'var(--amber)' },
              { label: 'MLflow Models', url: 'http://localhost:5000', color: 'var(--cyan)' },
              { label: 'Prometheus', url: 'http://localhost:9090', color: '#E53E3E' },
            ].map(({ label, url, color }) => (
              <a key={label} href={url} target="_blank" rel="noreferrer" style={{
                padding: '7px 14px', background: 'var(--bg-base)',
                border: `1px solid ${color}44`, borderRadius: 'var(--radius)',
